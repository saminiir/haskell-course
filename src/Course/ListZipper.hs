...
instance Functor ListZipper where
  (<$>) f (ListZipper a x b) = ListZipper (f <$> a) (f x) (f <$> b)
...
toList (ListZipper a x b) = reverse a ++ (x :. b)
...
withFocus f (ListZipper a x b) = ListZipper a (f x) b
...
findLeft p (ListZipper (a:.a') x b) = if p x then IsZ $ ListZipper (a :. a') x b else findLeft p (ListZipper a' a (x :. b))
findLeft p (ListZipper Nil x b) = if p x then IsZ $ ListZipper Nil x b else IsNotZ
...
findRight p (ListZipper a x (b:.b')) = if p x then IsZ $ ListZipper a x (b :. b') else findRight p (ListZipper (a ++ (x :. Nil)) b b')
findRight p (ListZipper a x Nil) = if p x then IsZ $ ListZipper a x Nil else IsNotZ
...
moveLeftLoop (ListZipper (a:.as) x bs) = ListZipper as a (x :. bs)
moveLeftLoop (ListZipper Nil x cs) = let ys = reverse cs 
                                         os = drop 1 ys ++ (x :. Nil)
                                         focus = headOr x ys
                                     in ListZipper os focus Nil
...
moveRightLoop (ListZipper as x (b:.bs)) = ListZipper (x :. as) b bs
moveRightLoop (ListZipper as x Nil) = let ys = reverse (x :. as)
                                          os = drop 1 ys
                                          focus = headOr x ys
                                      in ListZipper Nil focus os
...
moveLeft (ListZipper (a :. as) x bs) = IsZ $ ListZipper as a (x :. bs)
...
moveRight (ListZipper as x (b :. bs)) = IsZ $ ListZipper (x :. as) b bs 
...
swapLeft (ListZipper (a:.as) x bs) = IsZ $ ListZipper (x :. as) a bs
...
swapRight (ListZipper as x (b:.bs)) = IsZ $ ListZipper as b (x:.bs)
...
dropLefts (ListZipper _ x bs) = ListZipper Nil x bs
...
dropRights (ListZipper as x _) = ListZipper as x Nil
